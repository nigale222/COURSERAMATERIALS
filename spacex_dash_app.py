...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        site_success_counts.rename(columns={'Launch Site': 'Launch Site', 'class': 'Success Count'}, inplace=True)
        fig = px.pie(
            site_success_counts, 
            names='Launch Site', 
...
def update_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        filtered_df = spacex_df  # No specific site selected, so use the entire dataset
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
